...
func (u UserRepository) FindByID(id int) (interface{}, error) {
	...
	err := row.Scan(&user.ID, &user.Username, &user.FirstName, &user.LastName, &user.Email, &user.Document, &phantonZone, &user.Enabled, &user.CreationDate)
	return util.ResultData(user, model.User{}, err)
}

func (u UserRepository) Create(user model.User) (model.User, error) {
	err := db.GetDatasource().QueryRow("INSERT INTO golauth_user (username, first_name, last_name, email, document, password) VALUES ($1, $2, $3, $4, $5, $6) RETURNING id;",
