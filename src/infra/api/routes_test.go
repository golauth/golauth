...
func (s *RoutesSuite) SetupTest() {
	...
	s.repoFactory.EXPECT().NewUserRepository().AnyTimes().Return(mock.NewMockUserRepository(s.ctrl))
	s.repoFactory.EXPECT().NewRoleRepository().AnyTimes().Return(mock.NewMockRoleRepository(s.ctrl))
	s.repoFactory.EXPECT().NewUserRoleRepository().AnyTimes().Return(mock.NewMockUserRoleRepository(s.ctrl))
	s.repoFactory.EXPECT().NewUserAuthorityRepository().AnyTimes().Return(mock.NewMockUserAuthorityRepository(s.ctrl))
	s.NoError(err)
